...
按顺序执行：**数据节点 → 应用节点环境 → 导入 SQL → 后端 → 前端 → 验证**。
...
| 检查项 | 命令 / 预期 |
|--------|-------------|
| 后端健康 | `curl http://192.168.0.114/api/health` → `redis: UP` |
...
在 **数据节点** 上，按 `admin-backend/sql/` 中增量脚本顺序执行，或重新生成后导入：
...
| 前端白屏 + `Unexpected token '<'` | 旧 JS 缓存或 hash 不一致；重新 `npm run build` 并 rsync；勿用 `index_pc.html` |
| 前端白屏 + `SyntaxError: ID` 等 | vue-i18n 文案含 `#`、`@`、`<` 等特殊字符；需修 locale 后重新构建 |
...
| `deploy/nginx/ai-manager.conf` | Nginx 站点 |
