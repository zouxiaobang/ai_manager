...
- **左侧菜单**：首页、功能列表、笔记本、用户中心、权限中心、部署文档、存储中心、全局设置
...
专注计时页会每 2 秒从 `GET /api/pomodoro/session` 拉取设备上报的状态（写入 Redis），在设备端开始/暂停计时时与 Web 倒计时保持一致。副屏固件正在 `firmware/esp32_s3_sub_display` 重新开发中。
