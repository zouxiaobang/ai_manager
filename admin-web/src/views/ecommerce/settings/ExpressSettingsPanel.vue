<template>
  <el-form v-loading="loading" :model="form" label-width="150px" class="ec-settings-form">
    <el-form-item :label="t('ecommerce.monthlySettlement.headerRow')" required>
      <el-input-number v-model="form.headerRow" :min="1" :max="100" />
    </el-form-item>
    <el-form-item :label="t('ecommerce.monthlySettlement.dataStartRow')" required>
      <el-input-number v-model="form.dataStartRow" :min="2" :max="200" />
    </el-form-item>
    <el-form-item :label="t('ecommerce.settings.includeLabelPriceDefault')">
      <el-switch v-model="form.includeLabelPriceDefault" />
      <p class="ec-settings-form__hint">{{ t('ecommerce.settings.includeLabelPriceDefaultHint') }}</p>
    </el-form-item>
    <el-form-item>
      <el-button type="primary" :loading="saving" @click="onSave">{{ t('ecommerce.common.save') }}</el-button>
    </el-form-item>
  </el-form>
</template>

<script setup lang="ts">
import { onMounted, reactive, ref } from 'vue'
import { useI18n } from 'vue-i18n'
import { ElMessage } from 'element-plus'
import { fetchExpressSettings, saveExpressSettings } from '@/api/ecommerce/ecSettings'
import { useEcSettingsStore } from '@/stores/ecSettings'

const emit = defineEmits<{ saved: [updateTime?: string] }>()
const { t } = useI18n()
const ecSettings = useEcSettingsStore()

const loading = ref(false)
const saving = ref(false)
const form = reactive({
  headerRow: 1,
  dataStartRow: 2,
  includeLabelPriceDefault: false,
})

async function load() {
  loading.value = true
  try {
    const data = await fetchExpressSettings()
    form.headerRow = data.headerRow
    form.dataStartRow = data.dataStartRow
    form.includeLabelPriceDefault = !!data.includeLabelPriceDefault
    ecSettings.applyExpress(data)
  } finally {
    loading.value = false
  }
}

async function onSave() {
  if (form.dataStartRow <= form.headerRow) {
    ElMessage.warning(t('ecommerce.settings.importRowsInvalid'))
    return
  }
  saving.value = true
  try {
    const saved = await saveExpressSettings({ ...form })
    ecSettings.applyExpress(saved)
    ElMessage.success(t('ecommerce.settings.saveSuccess'))
    emit('saved', saved.updateTime)
  } finally {
    saving.value = false
  }
}

onMounted(() => { void load() })
</script>

<style scoped lang="scss">
.ec-settings-form {
  max-width: 560px;
}

.ec-settings-form__hint {
  margin: 6px 0 0;
  font-size: 12px;
  color: var(--el-text-color-secondary);
  line-height: 1.5;
}
</style>
