...
void Application::Run() {
    ...
    const EventBits_t ALL_EVENTS =
        ...
        MAIN_EVENT_STATE_CHANGED | MAIN_EVENT_RESET_LSE | MAIN_EVENT_TOGGLE_MENU;
    ...
    while (true) {
        ...
        if (bits & MAIN_EVENT_RESET_LSE) {
            HandleResetLSEvent();
        }

        if (bits & MAIN_EVENT_TOGGLE_MENU) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (cJSON_IsString(emotion)) {
                Schedule([display, emotion_str = std::string(emotion->valuestring)]() {
                    ESP_LOGI(TAG, "收到的表情 %s", emotion_str.c_str());
...
void Application::ResetLSState() { xEventGroupSetBits(event_group_, MAIN_EVENT_RESET_LSE); }

void Application::ToggleMenu() { xEventGroupSetBits(event_group_, MAIN_EVENT_TOGGLE_MENU); }
...
void Application::HandleResetLSEvent() {
    auto state = GetDeviceState();
    ESP_LOGI(TAG, "Is the device state is speaking or listener: %s",
             (state == kDeviceStateSpeaking || state == kDeviceStateListening) ? "是" : "否");
    ...
    if (protocol_ != nullptr) {
        protocol_->CloseAudioChannel();
        ESP_LOGI(TAG, "CloseAudio");
    }

    return;
}

void Application::HandleToggleMenuEvent() {
    auto& board = Board::GetInstance();
    auto display = board.GetDisplay();

    SpiLcdDisplay* lcd_display = dynamic_cast<SpiLcdDisplay*>(display);
    if (lcd_display) {
        lcd_display->OpenMenu();
    }
}

void Application::sendGlobalEvent(GlobalEvent event) {
    auto& board = Board::GetInstance();
    KyleBoard* kyle_board = dynamic_cast<KyleBoard*>(&board);
    if (kyle_board != nullptr) {
        kyle_board->handleEvent(event);
    }
}
