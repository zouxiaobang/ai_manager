...
void SpiLcdDisplay::SetEventCallback(std::function<void(MenuEvent)> cb) { event_cb_ = cb; }
void SpiLcdDisplay::CreateMenu() {
    DisplayLockGuard lock(this);

    menu_container_ = lv_obj_create(lv_screen_active());
    lv_obj_set_size(menu_container_, LV_HOR_RES, LV_VER_RES);
    lv_obj_set_style_bg_color(menu_container_, lv_color_hex(0x000000), 0);
    lv_obj_set_style_bg_opa(menu_container_, LV_OPA_90, 0);
    lv_obj_add_flag(menu_container_, LV_OBJ_FLAG_HIDDEN);

    // 使用 flex 布局
    lv_obj_set_flex_flow(menu_container_, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_style_pad_all(menu_container_, 20, 0);

    ShowRootMenu();
}
void SpiLcdDisplay::ShowRootMenu() {
    DisplayLockGuard lock(this);

    lv_obj_clean(menu_container_);


    menu_level_ = MenuLevel::ROOT;
    menu_size_ = menu_items_.size();

    for (auto& item : menu_items_) {
        lv_obj_t* btn = lv_btn_create(menu_container_);
...
void SpiLcdDisplay::UpdateMenuHighlight() {
    uint32_t count = lv_obj_get_child_cnt(menu_container_);

    for (uint32_t i = 0; i < count; i++) {
        lv_obj_t* btn = lv_obj_get_child(menu_container_, i);

        if (i == menu_index_) {
            lv_obj_set_style_bg_color(btn, lv_color_hex(0x00AAFF), 0);
        } else {
...
void SpiLcdDisplay::OpenMenu() {
    ESP_LOGI(TAG, "Open Menu");

    if (menu_container_ == nullptr)
        return;
    menu_open_ = true;
    ShowRootMenu();
    lv_obj_remove_flag(menu_container_, LV_OBJ_FLAG_HIDDEN);
}
void SpiLcdDisplay::CloseMenu() {
    if (menu_container_ == nullptr)
        return;
    menu_index_ = 0;
    menu_level_ = MenuLevel::ROOT;
...
void SpiLcdDisplay::MenuPrev() {
    if (!menu_open_)
        return;

    menu_index_--;
    if (menu_index_ < 0)
        menu_index_ = menu_size_ - 1;

    UpdateMenuHighlight();
}
void SpiLcdDisplay::MenuNext() {
    if (!menu_open_)
        return;

    menu_index_++;
    if (menu_index_ >= menu_size_)
        menu_index_ = 0;

    UpdateMenuHighlight();
}
void SpiLcdDisplay::MenuEnter() {
    if (!menu_open_)
        return;

    if (menu_level_ == MenuLevel::ROOT) {
        std::string item = menu_items_[menu_index_];
        if (item == "Deep Sleep") {
            event_cb_(MenuEvent::MENU_EVENT_DEEP_SLEEP);
        } else if (item == "Wifi Settings") {
            last_menu_index_ = menu_index_;
            ShowWifiMenu();
        } else if (item == "Volume") {
            last_menu_index_ = menu_index_;
            ShowVolumeMenu();
        } else if (item == "Toggle Theme") {
            ToggleTheme();
            CloseMenu();
        } else if (item == "Back") {
            menu_index_ = 0;
    ...
    } else if (menu_level_ == MenuLevel::VOLUME) {
        std::string item = volume_menu_items_[menu_index_];
        if (item == "Volume +") {
            event_cb_(MenuEvent::MENU_TYPE_VOLUME_UP);
        } else if (item == "Volume -") {
            event_cb_(MenuEvent::MENU_TYPE_VOLUME_DOWN);
        } else if (item == "Mute") {
            event_cb_(MenuEvent::MENU_TYPE_MUTE);
        } else if (item == "Back") {
            menu_index_ = last_menu_index_;
            ShowRootMenu();
        }
    } else if (menu_level_ == MenuLevel::WIFI_SETTING) {
        std::string item = wifi_menu_items_[menu_index_];
        if (item == "Reconnect") {
            event_cb_(MenuEvent::MENU_TYPE_WIFI_RECONNECT);
            CloseMenu();
        } else if (item == "Reset") {
            event_cb_(MenuEvent::MENU_TYPE_WIFI_RESET);
            CloseMenu();
        } else if (item == "Back") {
            menu_index_ = last_menu_index_;
            ShowRootMenu();
...
void SpiLcdDisplay::ShowVolumeMenu() {
    DisplayLockGuard lock(this);

    lv_obj_clean(menu_container_);

    menu_index_ = 0;
    menu_level_ = MenuLevel::VOLUME;
    menu_size_ = volume_menu_items_.size();

    for (auto& item : volume_menu_items_) {
        lv_obj_t* btn = lv_btn_create(menu_container_);
        lv_obj_set_width(btn, LV_PCT(100));
        lv_obj_set_height(btn, 40);
...
void SpiLcdDisplay::ShowWifiMenu() {
    DisplayLockGuard lock(this);

    lv_obj_clean(menu_container_);
    menu_index_ = 0;
    menu_level_ = MenuLevel::WIFI_SETTING;
    menu_size_ = wifi_menu_items_.size();

    for (auto& item : wifi_menu_items_) {
        lv_obj_t* btn = lv_btn_create(menu_container_);
        lv_obj_set_width(btn, LV_PCT(100));
        lv_obj_set_height(btn, 40);
...
void SpiLcdDisplay::ToggleTheme() {
    auto manager = &LvglThemeManager::GetInstance();
    auto current = static_cast<LvglTheme*>(current_theme_);

    if (current->name() == "light") {
        SetTheme(manager->GetTheme("dark"));
    } else {
        SetTheme(manager->GetTheme("light"));
    }

    CloseMenu();
}
void SpiLcdDisplay::MenuBack() {
    if (!menu_open_)
        return;

    CloseMenu();
}
bool SpiLcdDisplay::IsMenuOpened() { return menu_open_; }
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_width(chat_message_label_,
                     LV_HOR_RES - lvgl_theme->spacing(8));  // Subtract left and right padding
    ...
    lv_obj_set_style_text_align(chat_message_label_, LV_TEXT_ALIGN_CENTER,
                                0);  // Center text alignment
    lv_obj_set_style_text_color(chat_message_label_, lvgl_theme->text_color(), 0);
    lv_obj_align(chat_message_label_, LV_ALIGN_CENTER, 0,
                 0);  // Vertically and horizontally centered in bottom_bar_
