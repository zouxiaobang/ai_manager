...
public class NbTodoServiceImpl extends ServiceImpl<NbTodoItemMapper, NbTodoItem> implements NbTodoService {
    ...
    public NbTodoMutationVO update(Long id, NbTodoSaveRequest request) {
        ...
        if (request != null) {
            ...
            if (request.getPinned() != null) {
                item.setPinned(request.getPinned() ? 1 : 0);
            }
            applyScheduleFields(item, request);
    ...
    private void applyScheduleFields(NbTodoItem item, NbTodoSaveRequest request) {
        ...
        if (Boolean.TRUE.equals(request.getClearRepeatDays())) {
            item.setRepeatDays(null);
        } else if (request.getRepeatDays() != null) {
            item.setRepeatDays(normalizeRepeatDays(request.getRepeatDays()));
        }
    ...
    private String normalizeRepeatDays(String raw) {
    ...
    private NbTodoItemVO toVO(NbTodoItem item) {
        ...
        vo.setRepeatDays(item.getRepeatDays());
        vo.setSeriesId(item.getSeriesId());
        vo.setSortOrder(item.getSortOrder());
        vo.setPinned(item.getPinned() == null ? 0 : item.getPinned());
