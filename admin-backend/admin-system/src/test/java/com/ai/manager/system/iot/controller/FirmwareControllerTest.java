...
class FirmwareControllerTest {
    ...
    @Test
    void list_shouldReturnFirmwares() throws Exception {
        when(firmwareService.listFirmwares()).thenReturn(List.of(vo(1L, "2.2.1")));

        mockMvc.perform(get("/api/iot/firmware"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].version").value("2.2.1"));
    }
    ...
    @Test
    void otaRecords_shouldReturnRecords() throws Exception {
        OtaRecordVO record = new OtaRecordVO();
        record.setId(1L);
        record.setState("UPGRADING");
        when(firmwareService.listOtaRecords()).thenReturn(List.of(record));

        mockMvc.perform(get("/api/iot/firmware/ota-records"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].state").value("UPGRADING"));
    }
}
