...
constexpr char kHostFile[] = SD_MOUNT_POINT "/config/pixel_dog_host.txt";
...
void load_from_kconfig() {
  std::strncpy(s_host, CONFIG_PIXEL_DOG_API_HOST, sizeof(s_host) - 1);
  s_host[sizeof(s_host) - 1] = '\0';
  s_port = CONFIG_PIXEL_DOG_API_PORT;
}
}  // namespace
...
esp_err_t dog_api_config_load() {
  ...
  if (sd_storage_is_mounted() && parse_host_file()) {
    ESP_LOGI(TAG, "Pixel Dog backend from SD: %s:%d", s_host, s_port);
    return ESP_OK;
  }

  ESP_LOGI(TAG, "Pixel Dog backend from menuconfig: %s:%d", s_host, s_port);
