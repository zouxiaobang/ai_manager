...
constexpr size_t kLvglTaskStack = 16384;
constexpr uint32_t kLvglTaskDelayMs = 1;
...
void lvgl_add_psram_mem_pool() {
  ...
  if (pool == nullptr) {
    ESP_LOGW(TAG, "LVGL PSRAM pool alloc failed");
    return;
  }
...
esp_err_t display_init() {
  ESP_RETURN_ON_ERROR(board_io_init(), TAG, "Board IO init failed");
  ESP_RETURN_ON_ERROR(board_backlight_on(), TAG, "Backlight init failed");
  ...
  ESP_RETURN_ON_ERROR(esp_lcd_panel_mirror(panel_handle, PANEL_MIRROR_X, PANEL_MIRROR_Y), TAG, "Mirror failed");
  ...
  ESP_RETURN_ON_ERROR(esp_timer_create(&tick_timer_args, &lvgl_tick_timer), TAG, "Create tick timer failed");
  ESP_RETURN_ON_ERROR(esp_timer_start_periodic(lvgl_tick_timer, kLvglTickPeriodMs * 1000), TAG, "Start tick timer failed");
